---
name: kernel-generate
description: >
  指导 AI 智能体开发算子 kernel、功能测试、性能测试并迭代优化高性能 Triton kernel。
  当用户要求生成或实现 GPU 算子或 kernel、编写 Triton kernel、优化 kernel 性能、调试 kernel 正确性、
  运行功能测试或性能测试，或显式调用 kernel-generate 时使用。
---

# kernel-generate

## 概述

使用这个 skill 开发完整的 Triton kernel 实现：实现必须符合当前任务的算子接口，先通过功能测试证明正确性，再通过性能测试证明性能。流程需要保持通用：主流程和通用 kernel 方法不绑定任何具体实现仓库；所有具体仓库规则只允许写入 [references/custom_op_contract.md](references/custom_op_contract.md)。

## 资源加载

按需读取以下文件：

- [references/custom_op_contract.md](references/custom_op_contract.md)：实现前必须读取。这是唯一允许记录具体实现仓库规则、路径、测试框架、性能测试框架、代码风格和项目约束的文件。
- [references/kernel_impl_optimization.md](references/kernel_impl_optimization.md)：编写或修改 Triton 代码前必须读取。用于 kernel 设计、启动结构、索引、mask、访存行为和性能优化。
- [references/complex_operator_performance_patterns.md](references/complex_operator_performance_patterns.md)：优化 matmul、conv、attention/SDPA、复杂 GEMM-like 或多阶段归约算子时读取；尤其适用于基础 autotune 后仍不达标、需要 shape/dtype 分流、layout pack、split/reduce、persistent、descriptor/TMA、zero+atomic 或算法级重映射的场景。

## 执行规则

开始 kernel 实现、性能测试或性能优化前，先执行本 skill 附带的 `scripts/check_kernel_tools.py`，并向用户说明 PTX、SASS、NCU 工具是否可用。

1. 写代码前先读取 `custom_op_contract.md`，并把其中的具体仓库规则作为当前任务的硬约束。
2. 不要在 `SKILL.md`、`kernel_impl_optimization.md` 或 README 中新增具体仓库名、仓库路径、测试目录、benchmark 目录、注册方式或项目专属代码风格；这些内容只能写入 `custom_op_contract.md`。
3. 实现前先整理算子契约：公开 API、参数签名、输出约定、shape/dtype/layout 规则、参考实现、误差容忍度、功能测试覆盖、性能测试基准和通过条件。
4. 不要跳过功能测试。功能测试失败时，必须先分析并修复正确性，再运行性能测试。
5. 使用 `speedup = baseline_time / candidate_time`。所有必测性能测试用例的 `speedup >= 0.9` 才算性能合格。
6. **Autotune gate**：若 `custom_op_contract.md` 指定仓库级自动调优机制，并且算子存在 `BLOCK_*`、tile、split、`num_warps`、`num_stages`、grouping 等可调 META 参数，功能测试通过后、第一次正式性能结论前必须接入自动调优，运行目标 shape，并记录候选配置、命中配置和调优输出。固定 META 只允许用于正确性 bootstrap 或单轮受控探索；若 `speedup < 0.9` 且 tunable META 仍硬编码，下一步必须先接入或修正自动调优。
7. 性能迭代优化阶段不要在每次代码修改后运行完整功能测试；默认只运行性能测试并记录本轮未跑功能测试的原因。只有本轮改动明确触及接口、shape/dtype 处理、索引、mask、输出分配、别名/in-place 语义或数值计算路径时，才运行最小相关功能测试子集。
8. 所有必测性能用例合格后，必须再运行完整功能测试和最终性能测试，作为最终交付验证。
9. 性能不合格时，下一轮优化前必须创建或更新临时优化日志。
10. 性能目标是所有必测用例 `speedup >= 0.9`。如果性能测试环境可运行，任一必测用例仍不合格时，不得在少于 20 轮有效优化迭代后报告阻塞、放弃继续优化或交付未达标结论，除非用户明确要求停止。20 轮只是阻塞前的最低证据门槛；只要仍有可信优化路径，必须继续迭代。
11. 性能不合格时，不能只报告“未达标”。必须用实测证据说明主要瓶颈属于 benchmark 偏差、包装层/graph 开销、kernel 本体、baseline 算法差距、工具/环境阻塞，还是仍未归因。
12. 在向用户报告性能未达标、阻塞或放弃继续优化前，必须至少对一个代表性慢用例执行 PTX dump；如果 SASS 或 NCU 工具可用，还必须完成 SASS/NCU 分析。工具不可用时，必须在优化日志和最终回复中记录具体不可用原因和替代证据。
13. 只有在有 roofline/硬件指标、PTX/SASS/NCU 证据、baseline 算法证据或项目约束证据时，才可以判断“理论上或当前技术路径上难以达到”。否则只能说“当前实现尚未达标，尚不能证明理论不可达”。
14. 参数、tile、autotune 或局部代码调优耗尽后，如果已经识别出可信的算法路线（例如 fused split/reduce、分层归约、替代 GEMM 映射、persistent/stream-K、layout 变换、专用 shape kernel 或基准算法等价实现），这条路线必须作为下一轮优化继续执行，不能只在最终回复中列为“下一步”。只有当该路线受接口、正确性、硬件工具、项目约束或环境限制无法实施时，才可记录为阻塞证据。
15. 快但正确性失败的候选不得计入性能改进或保留在最终代码中。必须二选一：先用最小失败用例修复并复测正确性与性能；或回退/丢弃该候选，并在优化日志记录失败 dtype/shape、最大误差、根因假设和为什么不能作为可交付路径。如果修复正确性后性能不达标，这只能证明该局部路径被拒绝，仍需继续寻找新的正确算法路线。
16. 优先使用 `custom_op_contract.md` 指定的测试和性能测试约定。如果该文件没有规定，则从当前任务上下文中推断，并把新发现的具体仓库规则补充到该文件。
17. 只有当关键语义、接口要求或性能测试基准无法从代码、文档和 `custom_op_contract.md` 中推断，并且会阻塞实现或验证时，才向用户提问。

## 工作流

### 步骤 1：读取仓库契约并整理算子契约

读取 [references/custom_op_contract.md](references/custom_op_contract.md)。识别用户请求的算子、期望接口、参考实现、实现位置、测试位置、性能测试位置、依赖假设和可用 GPU 运行环境，并整理当前算子的具体契约。

算子契约至少包含：

- 接口：公开 API、参数签名、输入输出、in-place 或别名关系。
- 语义：参考实现、shape 规则、dtype 规则、layout/stride 规则、数值误差容忍度。
- 功能测试：必测用例、覆盖维度、测试命令。
- 性能测试：基准实现、候选实现、测试 shape、指标、通过条件和测试命令。

如果发现新的具体仓库规则，先补充到 `custom_op_contract.md`，再继续实现。如果接口、语义、误差容忍度或性能测试基准缺失，先记录显式假设；只有阻塞实现或验证的字段才向用户确认。

### 步骤 2：实现完整功能 kernel

读取 [references/kernel_impl_optimization.md](references/kernel_impl_optimization.md)。根据算子契约和 `custom_op_contract.md` 的硬约束实现 Triton kernel、Python 封装函数、启动网格、输出分配、dtype/shape 处理和集成代码。初版实现优先保证正确性，同时避免明显的性能反模式。初版可以用保守固定 META 参数做正确性 bootstrap；但功能测试通过后，如果仍存在可调 META 且仓库契约指定自动调优机制，必须先接入该机制，再进入扩展性能迭代。

### 步骤 3：编写并执行功能测试

根据算子契约和 `custom_op_contract.md` 编写功能测试。覆盖必需 shape、dtype、边界用例、layout 约束、广播或 stride 行为，以及数值误差容忍度。执行测试。

如果任何测试失败，先判断失败类别，再修改代码或测试，并重新执行功能测试。重复该过程直到功能测试通过。

常见失败类别：接口不匹配、shape/索引 bug、数值 bug、内存越界或别名关系问题、测试期望与契约不一致。

### 步骤 4：编写并执行性能测试

功能测试通过后，根据算子契约和 `custom_op_contract.md` 编写性能测试。将候选 Triton kernel 与约定基准实现对比，并为每个必测用例计算 speedup。

性能结论必须先确认 benchmark 有效：候选和 baseline 使用相同 dtype/layout/shape、相同 warmup/repeat/synchronization 策略，没有把编译、dump、一次性 graph capture 或异常处理计入候选侧耗时。如果排除某个 shape，必须记录触发原因、复现现象和为什么该 shape 不应计入必测集合。

第一次正式性能测试前必须完成适用的 Autotune gate。性能测试必须记录自动调优状态：是否使用仓库契约指定的自动调优入口、候选配置来源、命中配置、调优输出，以及无法使用自动调优时的具体原因。未过 Autotune gate 的硬编码 META 性能结果只能作为探索数据，不能作为最终性能结论。

如果所有必测用例的 `speedup >= 0.9`，则性能合格。报告完成前，重新执行最终功能测试和性能测试命令。最终性能结论不要默认使用 `TRITON_ALWAYS_COMPILE=1`，除非确认编译时间没有计入 benchmark。

### 步骤 5：性能迭代优化

如果任一必测用例的 `speedup < 0.9`，创建或更新临时优化日志。优先使用当前任务目录下的 `.kernel-generate/optimization-log-<operator>.md`；如果当前任务不适合写入临时文件，则使用 `/tmp/kernel-generate-optimization-log-<operator>.md`。性能优化只按下面闭环推进，直到所有必测用例达标或形成有证据的真实阻塞。

1. **Autotune gate**：如果仓库契约指定自动调优且 kernel 有 tunable META，先接入仓库自动调优，使用聚焦的候选集合覆盖目标 shape/dtype/layout，并用 `TRITON_PRINT_AUTOTUNING=1` 记录候选、命中配置和调优输出。单轮手动探索得到的好配置必须迁移回自动调优配置，不能长期留作硬编码最终值。
2. **Benchmark 与归因**：确认计时方式、编译缓存、warmup、同步、graph/capture 开销和 speedup 计算有效。将不达标原因归类为 benchmark 偏差、包装层/graph 开销、kernel 本体、baseline 算法差距、工具/环境阻塞或仍未归因。
3. **针对性迭代**：每轮选择一个慢用例，提出一个瓶颈假设，只做一个连贯修改：自动调优配置、kernel 代码、启动/包装层、dispatch 分流或算法结构。默认只跑相关性能测试；只有改动触及接口、shape/dtype、索引、mask、输出分配、别名/in-place 或数值路径时，才跑最小功能测试子集。
4. **工具分析**：autotune 后仍不达标且 kernel 瓶颈不清，或同一瓶颈连续 1-2 轮无改善时，立即按 [references/kernel_impl_optimization.md](references/kernel_impl_optimization.md) dump PTX；SASS 或 NCU 工具可用时继续反汇编/采集硬件指标。工具不可用时，记录具体命令、失败原因和替代证据。
5. **升级路线**：如果参数、autotune 和局部代码调优不能达标，必须实现下一个可信算法路线，例如 fused split/reduce、分层归约、替代 GEMM 映射、persistent/stream-K、layout 变换、专用 shape kernel 或基准算法等价实现。优化 matmul、conv、attention/SDPA 等复杂算子时，先读取 [references/complex_operator_performance_patterns.md](references/complex_operator_performance_patterns.md) 选择可验证的算法级路线，不能只把它写成“下一步”。

有效优化迭代必须在优化日志中用递增轮次记录，例如 `## Round 01 - ...`。一轮只有同时满足以下条件才计数：

- 记录本轮慢用例、开始 speedup 或耗时，以及当前 autotune 命中配置。
- 提出可验证的瓶颈假设。
- 做一次有针对性的配置、代码、包装层或算法修改；或者做一次能明确接受/拒绝某条优化路径的受控实验。
- 重新运行相关性能测试或受控 timing，并记录结果。
- 写明本轮结论、保留或回退的修改、功能测试状态、PTX/SASS/NCU 状态和下一轮假设。

不计入 20 轮最低门槛的内容：初始实现、算子契约整理、纯功能测试修复、单纯 benchmark 有效性说明、最终验证、环境检查、日志整理、未过 Autotune gate 的硬编码 META 反复调参，以及没有性能复测的尝试。性能测试环境可运行时，如果有效优化迭代少于 20 轮且仍有必测用例 `speedup < 0.9`，必须继续迭代。完成 20 轮后也不能自动停止；只有没有可信下一步且已经完成必要工具分析时，才可以报告阻塞或未达标。

### 参数调优耗尽后的升级规则

当实验证明现有 kernel 的 meta 参数、split 数、tile、num_warps、num_stages、partial dtype 或局部索引改写已经不能同时满足正确性和性能时，不得把“需要换算法路线”当作停止理由。必须立即进入算法级迭代，除非用户明确要求暂停。算法级迭代至少包括以下动作之一：

- 实现一个新的归约/分块/映射策略，并运行目标慢用例的正确性和性能测试。
- 将快速但错误的候选改造成正确版本，并量化修复正确性后的性能损失。
- 构造一个受控原型证明该算法路线不可行，并记录不可行原因、证据和下一个算法假设。

如果存在多个算法路线，优先选择最可能同时解决当前瓶颈和正确性约束的一条，而不是继续扩大低收益参数搜索。只有所有已识别可信算法路线都已实现、被证据拒绝或被环境/项目约束阻塞后，才可以报告“没有可信下一步”。

重复该循环直到性能合格。如果同一瓶颈反复出现，且已经完成深度调优或明确记录工具不可用，并且已经没有可信的下一步优化路径，则带上优化日志路径、当前最佳测试结果、瓶颈分类和证据向用户报告阻塞点。

### 步骤 6：报告结果

总结最终修改的算子文件、功能测试命令和结果、性能测试命令和结果、最终 speedup，以及临时优化日志路径（如果创建过）。同时说明算子契约中仍保留的假设。

如果性能未达标，最终回复必须包含：

- 当前最佳 speedup 和最慢用例。
- 已完成的有效优化迭代轮次和深度调优证据摘要；如果少于 20 轮，必须说明是用户要求停止还是性能测试环境不可运行。
- 已尝试的算法级路线，以及哪些路线因正确性、性能、项目约束或环境限制被拒绝。
- 主要瓶颈分类：benchmark 偏差、包装层/graph 开销、kernel 本体、baseline 算法差距、工具/环境阻塞或仍未归因。
- 为什么当前还不能达标；如果不能证明理论不可达，必须明确说“尚不能证明理论不可达”。
- 下一步最可信的优化方向，或者说明为什么没有可信路径。

## 示例

**示例 1：新增算子**

```text
用户说：“使用 kernel-generate 为当前仓库实现一个 Triton RMSNorm kernel。”
动作：
  1. 读取 custom_op_contract.md，确认当前仓库规则。
  2. 整理算子契约并实现 kernel。
  3. 添加功能测试并迭代直到通过。
  4. 添加性能测试并优化直到 speedup >= 0.9。
结果：得到一个已集成到当前仓库、功能测试和性能测试均通过的 Triton kernel。
```

**示例 2：优化已有慢 kernel**

```text
用户说：“优化这个 Triton softmax kernel；当前 speedup 只有 0.7。”
动作：
  1. 读取 custom_op_contract.md，确认功能契约和性能测试基准。
  2. 运行或重建失败的性能测试。
  3. 创建优化日志。
  4. 围绕具体瓶颈迭代修改，直到 speedup >= 0.9。
结果：得到优化后的 kernel，以及一份简短记录瓶颈、修改和性能测试结果的优化日志。
```

## 排障

| 问题 | 原因 | 处理 |
|------|------|------|
| 算子接口不明确 | 当前任务缺少公开封装、签名或文档 | 先从上下文整理草案契约，只向用户确认阻塞字段 |
| 具体仓库规则散落在其他文件 | 文档边界被破坏 | 将仓库名、路径、测试框架、性能测试框架和项目约束迁回 `custom_op_contract.md` |
| 优化后功能测试失败 | 优化改动影响了索引、mask、dtype、别名关系或误差行为 | 用最小失败用例复现，先修复正确性，再继续性能测试 |
| speedup 低于 0.9 | benchmark、包装层、kernel 本体或 baseline 算法差距均可能导致 | 追加本轮优化日志，按分层归因收集证据；报告失败前必须完成 PTX，工具可用时完成 SASS/NCU |
