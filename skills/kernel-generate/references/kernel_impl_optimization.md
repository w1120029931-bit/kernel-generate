...
如果仓库契约强制 autotune，autotune 是功能正确后的第一性能优化步骤；否则只有当算子契约和项目约束允许时才使用 autotune。候选配置要聚焦；过宽的 autotune 配置网格会拖慢测试，也会模糊主要瓶颈。
...
当必需的 autotune 已接入后仍有用例 `speedup < 0.9` 且瓶颈假设不清晰，或同一瓶颈连续 1-2 轮无改善时，升级到证据驱动的深度调优。报告性能未达标、阻塞或放弃继续优化前，也必须完成本节的可执行部分。核心原则：PTX 能提示问题，但不能单独证明瓶颈；SASS 更接近 GPU 实际执行；Nsight Compute 指标用于判断真实硬件瓶颈。
...
7. 若改动触及正确性风险项，重新运行最小功能测试子集；始终重新运行相关性能测试。
...
1. 从测得的慢用例和当前 autotune 命中配置出发。
2. 明确一个瓶颈假设。
3. 只修改自动调优配置、kernel、启动/包装层或算法结构中的一个连贯部分。
4. 若改动触及正确性风险项，运行最小功能测试子集；否则记录跳过原因。
5. 重新运行相关性能测试用例。
6. 将本轮结果、keep/revert 决策和下一轮假设记录到临时优化日志。
